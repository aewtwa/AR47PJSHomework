...
int main()
{
    ...
    // ex) = 연산자는 이항 연산자입니다.
    // Left 3을 사용해서 2개의 메모리 영역이 있어야만 동작하기때문에 이항 연산자라고합니다.
    ...
    Result = 1 + 2 * 3;
    // () 괄호 연산자.
    // 연산순서를 프로그래머가 지정하게 해줄 수 있는 연산자 입니다.
    Result = (1 + 2) * 3;
    ...
    // 논리 연산자
    bResult = true && true; // 둘다 참이라면 참, 하나만 거짓이여도 거짓. and
    bResult = true && false;
    bResult = false && false;

    bResult = true || true; // 둘중 하나라도 참이라면 참, 둘다 거짓이면 거짓. or
    bResult = true || false;
    bResult = false || false;
}